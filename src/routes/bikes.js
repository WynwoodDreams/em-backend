...
router.put('/:id', authenticateToken, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `UPDATE bikes SET make = COALESCE($1, make), model = COALESCE($2, model), 
       year = COALESCE($3, year), vin = COALESCE($4, vin), mileage = COALESCE($5, mileage),
       color = COALESCE($6, color), engine = COALESCE($7, engine), power = COALESCE($8, power),
       image = COALESCE($9, image), status = COALESCE($10, status), updated_at = NOW()
...
router.post('/:id/photos', authenticateToken, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `INSERT INTO bike_photos (bike_id, photo_url, is_primary)
